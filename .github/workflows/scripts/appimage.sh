...
cp /lib/i386-linux-gnu/{libpthread.so.0,libdl.so.2,libz.so.1,libXau.so.6,libXdmcp.so.6,libXext.so.6,librt.so.1,libbsd.so.0,libresolv.so.2,libgpg-error.so.0,libOpenGL.so} AppDir/usr/lib32
