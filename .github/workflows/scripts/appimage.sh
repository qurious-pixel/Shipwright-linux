...
cp /lib/i386-linux-gnu/{ld-linux.so.2,libGL.so.1,libstdc++.so.6,libm.so.6,libgcc_s.so.1,libc.so.6,libxcb.so.1,libGLdispatch.so.0,libGLX.so.0,\
   libpthread.so.0,libdl.so.2,libz.so.1,libXau.so.6,libXdmcp.so.6,libXext.so.6,librt.so.1,libbsd.so.0,libresolv.so.2,libgpg-error.so.0,libOpenGL.so} AppDir/usr/lib32
