...
int dbg_camera_pad_num = 0; //change 0 to the controller port you want to use.
#define ACTION_E 0
...
void func_800B44E0(DbCamera* dbCamera, Camera* cam) {
    ...
    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CRIGHT)) {
...
void DbCamera_Update(DbCamera* dbCamera, Camera* cam) {
    ...
    *sp90 = temp_s6->pos;
    ...
    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num+1].press.button, BTN_Z)) {//Not working at the moment!
        dbCamera->unk_00++;
        dbCamera->unk_00 %= 3;
        dbCamera->unk_38 = 1;
        dbCamera->unk_44 = 0;
    ...
    if ((dbCamera->unk_38 != 0) || dbCamera->unk_3C) {
        ...
        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_B | BTN_L)) {
            sp104.r += temp_f2;
            ...
            dbCamera->unk_40 = 7;
        } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_B)) {
            spFC = sp104;
            spFC.r = temp_f2;
            if (!D_80161144) {
                spFC.yaw = sp104.yaw;
            ...
            dbCamera->unk_40 = 0xB;
        } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_A | BTN_L)) {
            sp104.r -= temp_f2;
            if (sp104.r < 10.0f) {
                sp104.r = 10.0f;
            }
            ...
            dbCamera->unk_40 = 8;
        } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_A)) {
            spFC = sp104;
            spFC.r = -temp_f2;
            if (!D_80161144) {
                spFC.yaw = sp104.yaw;
    ...
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DDOWN | BTN_L)) {
        spFC = sp104;
        spFC.r = temp_f2;
        spFC.pitch = 0;
        if (!D_80161144) {
        ...
        dbCamera->unk_40 = 1;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DUP | BTN_L)) {
        spFC = sp104;
        spFC.r = -temp_f2;
        spFC.pitch = 0;
        if (!D_80161144) {
        ...
        dbCamera->unk_40 = 2;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DUP)) {
        spFC = sp104;
        spFC.r = temp_f2;
        spFC.pitch = 0x3FFF;
        spFC.yaw = sp104.yaw;
        ...
        dbCamera->unk_40 = 3;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DDOWN)) {
        spFC = sp104;
        spFC.r = temp_f2;
        spFC.pitch = -0x3FFF;
        spFC.yaw = sp104.yaw;
        ...
        dbCamera->unk_40 = 4;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, (BTN_DRIGHT | BTN_L)) ||
               CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DRIGHT)) {
        spFC = sp104;
        spFC.r = temp_f2;
        spFC.pitch = 0;
        if (!D_80161144) {
        ...
        dbCamera->unk_40 = 5;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, (BTN_DLEFT | BTN_L)) ||
               CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DLEFT)) {
        spFC = sp104;
        spFC.r = temp_f2;
        spFC.pitch = 0;
        if (!D_80161144) {
        ...
        dbCamera->unk_40 = 6;
    } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_B | BTN_L)) {
    ...
    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_R)) {
        if (dbCamera->unk_00 == 0) {
            ...
            *sp80 = cam->eye;
            ...
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_R) &&
                CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_L)) {
                Audio_PlaySoundGeneral(NA_SE_SY_CANCEL, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
                dbCamera->sub.nPoints = dbCamera->sub.unkIdx + 1;
                func_800B4088(dbCamera, cam);
            } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_R)) {
        ...
        temp_f0_5 = sGlobalCtx->state.input[dbg_camera_pad_num].rel.stick_y;
        temp_f2_2 = sGlobalCtx->state.input[dbg_camera_pad_num].rel.stick_x;
        ...
        if (dbCamera->unk_00 == 1) {
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_CRIGHT)) {
    ...
    if (dbCamera->unk_00 == 1) {
        ...
        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_CRIGHT) &&
            ...
            } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CLEFT)) {
                Audio_PlaySoundGeneral(NA_SE_SY_CURSOR, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
                dbCamera->sub.unk_08 = (dbCamera->sub.unk_08 + 1) % 3;
            }

            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CUP) &&
                CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_L)) {
            ...
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_L) &&
                CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CDOWN)) {
                Audio_PlaySoundGeneral(NA_SE_IT_SWORD_IMPACT, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
                if (dbCamera->sub.unkIdx < (dbCamera->sub.nPoints - 1)) {
                    dbCamera->sub.unkIdx++;
                } else {
                ...
                if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CDOWN)) {
                    Audio_PlaySoundGeneral(NA_SE_IT_SWORD_IMPACT, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                           &D_801333E8);
                    if (dbCamera->sub.unkIdx < (dbCamera->sub.nPoints - 1)) {
                        dbCamera->sub.unkIdx++;
            ...
            switch (dbCamera->sub.unk_08) {
                ...
                case 1:
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DUP)) {
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DDOWN)) {
                        Audio_PlaySoundGeneral(NA_SE_SY_ATTENTION_ON, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                               &D_801333E8);
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DLEFT)) {
                        Audio_PlaySoundGeneral(NA_SE_SY_ATTENTION_ON, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                               &D_801333E8);
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DLEFT)) {
                        if ((D_8012D10C++ % 5) == 0) {
                            Audio_PlaySoundGeneral(NA_SE_SY_ATTENTION_ON, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                                   &D_801333E8);
                        }
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DRIGHT)) {
                        Audio_PlaySoundGeneral(NA_SE_SY_ATTENTION_ON, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                               &D_801333E8);
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_DRIGHT)) {
                        if ((D_8012D10C++ % 5) == 0) {
                            Audio_PlaySoundGeneral(NA_SE_SY_ATTENTION_ON, &D_801333D4, 4, &D_801333E0, &D_801333E0,
                                                   &D_801333E8);
                        }
        ...
        dbCamera->fov = 60.0f;
        dbCamera->rollDegrees = dbCamera->roll * 1.40625f;
        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CLEFT)) {
            Audio_PlaySoundGeneral(NA_SE_SY_CURSOR, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
            ...
            switch (dbCamera->sub.demoCtrlMenu) {
                ...
                case DEMO_CTRL_MENU(ACTION_CLEAR, MENU_INFO): {
                    if ((1 << sCurFileIdx) & sMempakFiles) {
                        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DLEFT) ||
                            CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DRIGHT)) {
                        ...
                        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_A)) {
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_B)) {
                        Audio_PlaySoundGeneral(NA_SE_SY_CANCEL, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
                        dbCamera->sub.demoCtrlMenu = 0;
                        return 1;
                    }
                ...
                case DEMO_CTRL_MENU(ACTION_CLEAR, MENU_SUCCESS): {
                    ...
                    if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_A) ||
                        CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_B)) {
                ...
                default: {
                    ...
                    block_1:
                        ...
                        if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_B) ||
                            CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DUP) ||
                            CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DDOWN)) {
        ...
        default: {
            ...
            if (sDbCamAnim.unk_0A != 0) {
                ...
                if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CUP)) {
                    ...
                    sDbCamAnim.keyframe = 0;
                    sDbCamAnim.unk_04 = 0;
                } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CDOWN)) {
                    if (D_8016110C < 14) {
                        D_8016110C++;
                    }
            ...
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CRIGHT) && CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_DRIGHT)) {
                D_8015FCC8 = 0;
            ...
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_L)) {
                if (sp74[sCurFileIdx] == '?') {
                    sLastFileIdx = -1;
                    D_801612EA = '*';
                } else {
                ...
                if (1) {}
            } else if (!CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_L)) {
                if (sLastFileIdx != -1) {
            ...
            if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].cur.button, BTN_L) &&
                CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CRIGHT)) {
                for (i = 0; i < ARRAY_COUNT(sDbCameraCuts) - 1; i++) {
                ...
                DbCamera_PrintAllCuts(cam);
                       ...
                       CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CLEFT)) {
                Audio_PlaySoundGeneral(NA_SE_SY_GET_RUPY, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
                for (i = 0; i < ARRAY_COUNT(sDbCameraCuts) - 1; i++) {
            ...
            } else if (CHECK_BTN_ALL(sGlobalCtx->state.input[dbg_camera_pad_num].press.button, BTN_CRIGHT)) {
                sDbCamAnim.curFrame = 0.0f;
                sDbCamAnim.keyframe = 0;
